	...
	<script type="text/javascript">
		$(function(){
			$("#allusers").datagrid({
				...
				url:"carlist.action",
				columns:[[    
				          {field:'number',title:'车牌号码',width:150,align:'center'},    
				          {field:'type',title:'车辆类型',width:150,align:'center'},
				          {field:'length',title:'车身长度(m)',width:150,align:'center'},
				          {field:'tonnage',title:'车辆吨位(T)',width:150,align:'center'},
				          {field:'fkTeamid',title:'所属车队',width:150,align:'center',
				        	  formatter: function(value,row,index){
				        		
					  		  		return getTeamName(value);
					  			}
				          },
				          {field:"state",title:'工作状态', width:150,align:'center',
					  		  	formatter: function(value,row,index){
					  		  		if(value = 1){
					  		  			return "空闲中";
					  		  		}else{
				          ...
				          {field:"truckid",title:'操作', width:150,align:'center',
					  		  	formatter: function(value,row,index){
					  		  		var str = '<a href=initUpdateDriver.action?truckid='+value+' name="update" class="easyui-linkbutton" ></a>'; 
		...
		function fdelete(value){
			$.messager.confirm('确认删除', '您确定要删除该车辆吗？', function(r){
				if (r){
					 top.location="deleteTruck.action?truckid="+value+"";
				}
			});
		}
		function getTeamName(value){
			var res;
			$.post("gettn.action?id="+value,function(data,status){
		        res = data;
